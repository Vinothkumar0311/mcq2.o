...
const AdminTestReports = () => {
    ...
    try {
      ...
      if (data.success && data.results) {
        ...
        const processedTests = Object.values(testGroups).map(group => {
          ...
          const highestScore = totalStudents > 0 ? Math.max(...students.map(s => s.percentage)) : 0;
  ...
  const generateReport = async (testId: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/reports/generate-test-report/${testId}`, {
        method: 'POST',
        headers: {
  ...
  const downloadReport = async (testId: string, format: 'excel' | 'csv') => {
    try {
      ...
      a.href = url;
      a.download = `test-report-${testId}.${format === 'excel' ? 'pdf' : 'csv'}`;
  ...
  const openTestModal = async (test: TestReport) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/test-results?testId=${test.testId}`);
      ...
      if (data.success && data.results) {
        ...
        const formattedResults = data.results.map(result => ({
          ...
          codingResults: result.type === 'section-based' ? [
            {
              ...
              grade: result.percentage >= 90 ? 'A' : result.percentage >= 80 ? 'B' : result.percentage >= 70 ? 'C' : 'D',
              language: 'Java'
            }
          ] : []
        }));
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `${studentName.replace(/\s+/g, '_')}_${selectedTest.testName.replace(/\s+/g, '_')}_Report.pdf`;
          document.body.appendChild(a);
          a.click();
          window.URL.revokeObjectURL(url);
          document.body.removeChild(a);
      ...
      if (student.mcqResults && student.mcqResults.totalQuestions > 0) {
        ...
        if (student.mcqAnswers && student.mcqAnswers.length > 0) {
          student.mcqAnswers.forEach((answer, index) => {
            ...
            pdf.setFont('helvetica', 'normal');
            pdf.text(`Selected: ${answer.selectedAnswer || 'Not answered'}`, 15, y + 16);
            pdf.text(`Correct: ${answer.correctAnswer}`, 15, y + 24);
            
            if (answer.explanation) {
              const explanationLines = pdf.splitTextToSize(`Explanation: ${answer.explanation}`, 180);
  ...
  const downloadOverallReport = async () => {
    try {
      // Try API first, fallback to local generation
      try {
        const response = await fetch(`${API_BASE_URL}/api/reports/download-test-report/${selectedTest.testId}`);
      ...
      studentResults
        ...
        .forEach((student, index) => {
          ...
          pdf.text(`${student.percentage}%`, 130, y + 7);
      ...
      if (mcqStudents.length > 0) {
        ...
        const avgMcqCorrect = Math.round(mcqStudents.reduce((sum, s) => sum + s.mcqResults.correctAnswers, 0) / mcqStudents.length);
        const mcqAccuracy = Math.round((avgMcqCorrect / totalMcqQuestions) * 100);
      ...
      // Coding Analysis
      const codingStudents = studentResults.filter(s => s.codingResults && s.codingResults.length > 0);
      if (codingStudents.length > 0) {
        ...
        const avgTestCasesPassed = Math.round(
          codingStudents.reduce((sum, s) => 
            sum + s.codingResults.reduce((cSum, cr) => cSum + cr.testCasesPassed, 0), 0
          ) / codingStudents.length
        );
        const avgTotalTestCases = Math.round(
          codingStudents.reduce((sum, s) => 
            sum + s.codingResults.reduce((cSum, cr) => cSum + cr.totalTestCases, 0), 0
          ) / codingStudents.length
        );
        const codingSuccessRate = avgTotalTestCases > 0 ? Math.round((avgTestCasesPassed / avgTotalTestCases) * 100) : 0;
      ...
      pdf.save(`${selectedTest.testName.replace(/\s+/g, '_')}_Comprehensive_Overall_Report.pdf`);
      
      toast({
        title: "Success",
        description: "Comprehensive overall test report downloaded successfully",
      });
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Download and manage student test reports</p>
          </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm text-gray-600">
                    ...
                    {test.averageScore && (
                      <div className="flex items-center gap-2">
                        <span className="text-blue-600 font-medium">Avg: {test.averageScore}%</span>
                      </div>
                    )}
                  </div>
                      ...
                      {test.hasReports ? (
                        <>
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigate(`/admin/test-reports/${test.testId}/students`)}
                        ...
                        <div className="flex gap-2">
                          {test.completedStudents > 0 ? (
                            ...
                            <>
                              ...
                              <span className="text-sm text-gray-500 italic flex items-center">No completed tests</span>
                            </>
                          )}
                        </div>
                      )}
                ...
                {modalLoading ? (
                  ...
                  <div className="space-y-4">
                    <div className="text-center mb-6">
                      <h3 className="text-xl font-bold text-gray-900 mb-2">🏆 Test Leaderboard</h3>
                      <p className="text-gray-600">Top performers ranked by score</p>
                    </div>
                      ...
                      .map((student, index) => {
                        ...
                        return (
                          <div key={index} className={`p-4 rounded-lg border-2 transition-all ${
                            rank === 1 ? 'bg-yellow-50 border-yellow-300 shadow-lg' :
                            rank === 2 ? 'bg-gray-50 border-gray-300 shadow-md' :
                            rank === 3 ? 'bg-orange-50 border-orange-300 shadow-md' :
                            'bg-white border-gray-200'
                          }`}>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${
                                  isTopThree ? 'bg-white shadow-md' : 'bg-gray-100'
                                }`}>
                                  {medalEmoji || `#${rank}`}
                                </div>
                                <div>
                              ...
                              <div className="flex items-center gap-6">
                                <div className="text-center">
                                  <div className={`text-2xl font-bold ${
                                    ...
                                    student.percentage >= 70 ? 'text-yellow-600' : 'text-red-600'
                                  }`}>
                                    {student.percentage}%
                                  </div>
                                ...
                                {student.codingResults && student.codingResults.length > 0 && (
                                  <div className="text-center">
                                    <div className="text-sm font-semibold text-purple-600">
                                      Coding: {student.codingResults.reduce((sum, cr) => sum + cr.testCasesPassed, 0)}/
                                      {student.codingResults.reduce((sum, cr) => sum + cr.totalTestCases, 0)}
                                    </div>
                                    <p className="text-xs text-gray-500">
                                      {Math.round((student.codingResults.reduce((sum, cr) => sum + cr.testCasesPassed, 0) / 
                                      student.codingResults.reduce((sum, cr) => sum + cr.totalTestCases, 0)) * 100)}% test cases
                                    </p>
                                  </div>
                                )}
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                      <Card>
                        <CardContent className="p-4 text-center">
                          <div className="text-2xl font-bold text-blue-600">{studentResults.length}</div>
                      ...
                      <Card>
                        <CardContent className="p-4 text-center">
                          <div className="text-2xl font-bold text-green-600">
                            {studentResults.filter(s => s.percentage >= 60).length}
                          </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {/* Coding Section */}
                      {studentResults.some(s => s.codingResults && s.codingResults.length > 0) && (
                        <Card className="border-l-4 border-l-purple-500">
                          ...
                          <CardContent>
                            <div className="space-y-3">
                              {studentResults
                                .filter(s => s.codingResults && s.codingResults.length > 0)
                                .map((student, index) => {
                                  const totalTestCases = student.codingResults.reduce((sum, cr) => sum + cr.totalTestCases, 0);
                                  const passedTestCases = student.codingResults.reduce((sum, cr) => sum + cr.testCasesPassed, 0);
